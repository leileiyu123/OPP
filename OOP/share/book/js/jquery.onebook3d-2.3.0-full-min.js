...
! function(t) {
	t.onebook = function(e, i, s) {
		var a = "ONEBOOK3DGLOBALDATASTORAGE",
			s = s && s !== t && s.size ? s : !1;
		window[a] || (window[a] = {
			FLIPS: {},
			NUMBER: 0,
		...
		var n = {
				...
				initWithData: function(e) {
					var i = this,
						...
						n = {
							...
							defineLanguage: function(t) {
								var e = ONEBOOK3D_ADDITIONAL_LANGUAGES,
									i = {
										en: 0,
										ru: 1
									};
								if(e && e.length > 0)
									for(var s = 0; s < e.length; s++) i[e[s].lng] = s + 2;
					...
					if(h && h.length > 0)
						for(var r = 0; r < h.length; r++) this.LNG.save.push(h[r].save), this.LNG.light.push(h[r].light), this.LNG.dark.push(h[r].dark), this.LNG.prev.push(h[r].prev), this.LNG.next.push(h[r].next), this.LNG.zoom.push(h[r].zoom), this.LNG.zoomclose.push(h[r].zoom), this.LNG.slope.push(h[r].slope), this.LNG.fromfullscreen.push(h[r].fromfullscreen), this.LNG.tofullscreen.push(h[r].tofullscreen), this.LNG.close.push(h[r].close), this.LNG.left.push(h[r].left), this.LNG.right.push(h[r].right), this.LNG.toStart.push(h[r].toStart), this.LNG.toEnd.push(h[r].toEnd), this.LNG.hlpGoToHomelink.push(h[r].goToHomelink), this.LNG.hlpClickToOpen.push(h[r].clickToOpen), this.LNG.hlpUseMousewheel.push(h[r].useMousewheel + "<br>" + h[r].forEasyViewing), this.LNG.hlpUseMousewheelGL.push([h[r].useMousewheel, h[r].forEasyViewing]), this.LNG.askSaveImage.push(h[r].askSaveImage), this.LNG.askSaveLinks.push('<a href="#">' + h[r].left + "</a> " + h[r].or + ' <a href="#">' + h[r].right + "</a>");
					this.BTNS_OFFSET = {
						save: [
							["-100px"]
						],
				...
				getEnviron: function() {
					var t = navigator.userAgent,
						e = -1 != t.indexOf("Chrome"),
						i = -1 != t.indexOf("Safari"),
						s = -1 != t.indexOf("OPR"),
						a = -1 != t.indexOf("WebKit"),
						n = {
				...
				addStyle: function() {
					...
					var i = function(e) {
						...
						s = "#" + this.divNames.background,
						a = "#" + this.divNames.book_stage,
						n = "#" + this.divNames.book_container,
						o = "#" + this.divNames.glossy_container,
						h = "#" + this.divNames.book_help_layer,
						r = "#" + this.divNames.book_loader,
						d = "." + this.divNames.book_spread_title,
						l = "#" + this.divNames.book_page_numbers,
						g = "#" + this.divNames.book_icons_panel,
						A = "#" + this.divNames.zoom_layer,
						p = "." + this.divNames.zoom_content,
						c = "." + this.divNames.zoom_title_spread,
						f = "#" + this.divNames.zoom_waiter;
					saveMenu = "." + this.divNames.save_menu;
					var E = this.CFG.ZINDEX,
						S = "";
				...
				getStage: function() {
					var e = this.divNames.book_stage,
						i = this.divNames.book_container,
						s = this.divNames.glossy_container,
						a = this.divNames.book_loader,
						n = this.divNames.zoom_waiter,
						o = t("#" + e);
					o.size() && o.remove();
				...
				exit: function() {
					...
					var e = this.options.parentBook,
						i = this.CURRENT.spread,
						s = this.CURRENT.slope_mode,
						n = function() {
				...
				restart: function(t) {
					if("reposition" === t) {
						var e = this.TARGETSIZE,
							i = this.STAGE,
							s = Math.round((e.width - i.width) / 2 + e.left),
							a = this.getIconsPanelWidth(),
							n = Math.round((e.width - a) / 2 + e.left),
						...
						this.$gStage.css({
							...
							left: o
						}), this.prepareBookSize(), this.calculateBounds()
				...
				gotoSpread: function(t) {
					var e = this,
						i = this.getTotalSheets();
					if(this.GLOSSY && (e.GL_LINK_TO_START = !1, e.$gStage.removeClass("pointer")), !this.ZOOM_MODE && !this.ANIMATION_FLAG) {
						0 === t ? (this.START_FROM_END = !0, this.toggleBook()) : t === i ? (this.START_FROM_END = !1, this.toggleBook()) : (this.CURRENT.spread = t, this.GLOSSY || (this.$gHelpLayer.find(".hlpLeftSide").css({
							opacity: 0
					...
					var e = this,
						...
						s = {
							...
							drawHomeLink: function() {
								if(i.HOME_LINK) s.drawHelpArrow();
								else {
									var t = new Image;
									t.onload = function() {
							...
							drawHelpArrow: function() {
								if(i.HELP_ARROW) s.drawIconsBig();
								else {
									var t = new Image;
									t.onload = function() {
										var t = document.createElement("canvas");
				...
				buildIconsPanel: function() {
					var e = this,
						i = this.CFG.ICONS_PANEL,
						s = (this.STAGE, this.CFG.BOOK_SIZE, this.divNames.book_icons_panel),
						a = this.divNames.icons_container,
						n = this.divNames.book_spread_title,
						o = this.DARK_MODE ? "" : " light",
						h = {
							build: function() {
								if(e.NO_ONE_TITLES) {
									var s = i[r].noTitle;
						...
						r = this.PANEL_TYPE,
						d = this.getIconsPanelWidth(),
						l = this.getIconsPanelHeight();
					if(r) {
						var g = h.build(),
							A = t("#" + s);
						A.size() && A.remove();
						...
						var f = {
								...
								fullScreen: function() {
									if(e.$gSaveMenu.hide(), !e.ZOOM_MODE)
										if(e.BOOK_INTERNAL) {
						...
						if("tiny" === r) {
							...
							u.find(">div").append(this.buttonCreate("Save", f.save)).append(p.clone()), u.find(">div").append(this.pageNumbersCreate(r)).append(p.clone()), c.append(u.css({
								"float": "left"
							}));
							var S = this.cloneIconsSet();
							...
							R.find(">div").append(this.buttonCreate("togglebook", f.toggleBook)).append(p.clone()), R.find(">div").append(this.buttonCreate("Slope", f.slope)).append(p.clone()), R.find(">div").append(this.buttonCreate(E, f.fullScreen)), c.append(R.css({
								"float": "right"
							})), this.$gButtonsTitle = A.find(".btn_title")
						} else if("middle" === r) {
							var u = this.cloneIconsSet();
							u.find(">div").append(this.buttonCreate("Save", f.save)).append(p.clone()), u.find(">div").append(this.buttonCreate("Skin", f.skin)).append(p.clone()), u.find(">div").append(this.pageNumbersCreate(r)).append(p.clone()), c.append(u.css({
								"float": "left"
							}));
							var S = this.cloneIconsSet();
							S.find(">div").append(this.buttonCreate("Prev", f.prev)).append(p.clone()), S.find(">div").append(this.buttonCreate("Zoom", f.zoom)).append(p.clone()), S.find(">div").append(this.buttonCreate("Next", f.next)), c.append(S), S.css({
							...
							var R = this.cloneIconsSet();
							R.find(">div").append(this.buttonCreate("togglebook", f.toggleBook)).append(p.clone()), R.find(">div").append(this.buttonCreate("Slope", f.slope)).append(p.clone()), R.find(">div").append(this.buttonCreate(E, f.fullScreen)), R.find(">div").append(p.clone()).append(this.buttonCreate("Close", f.close)), c.append(R.css({
								"float": "right"
							})), this.$gButtonsTitle = A.find(".btn_title")
						} else if("large" === r) {
							var u = this.cloneIconsSet();
							u.find(">div").append(this.buttonCreate("Save", f.save)).append(p.clone()), u.find(">div").append(this.buttonCreate("Skin", f.skin)).append(p.clone()), c.append(u.css({
								"float": "left"
							}));
							var S = this.cloneIconsSet();
							S.find(">div").append(this.buttonCreate("Prev", f.prev)).append(p.clone()), S.find(">div").append(this.buttonCreate("Zoom", f.zoom)).append(p.clone()), S.find(">div").append(this.buttonCreate("Next", f.next)), c.append(S), S.css({
							...
							var R = this.cloneIconsSet();
							R.find(">div").append(this.buttonCreate("togglebook", f.toggleBook)).append(p.clone()), R.find(">div").append(this.buttonCreate("Slope", f.slope)), e.$TARGET || R.find(">div").append(p.clone()).append(this.buttonCreate("Close", f.close)), c.append(R.css({
								"float": "right"
							})), this.$gButtonsTitle = A.find(".btn_title")
						}
				...
				toggleBook: function() {
					if(!this.ZOOM_MODE && !this.ANIMATION_FLAG) {
						...
						this.START_FROM_END ? (this.START_FROM_END = !1, this.CURRENT.spread = 0, this.GLOSSY ? t.left && t.right && (t.left.material.opacity = 1, t.right.material.opacity = 0) : (this.$gHelpLayer.find(".hlpLeftSide").css({
							opacity: 1
						}), this.$gHelpLayer.find(".hlpRightSide").css({
							opacity: 0
						}))) : (this.START_FROM_END = !0, this.CURRENT.spread = this.getTotalSheets(), this.GLOSSY ? t.left && t.right && (t.left.material.opacity = 0, t.right.material.opacity = 1) : (this.$gHelpLayer.find(".hlpLeftSide").css({
							opacity: 0
						}), this.$gHelpLayer.find(".hlpRightSide").css({
							opacity: 1
						})));
				...
				toggleSkin: function() {
					var e = this,
						i = t("#" + this.divNames.book_page_numbers),
						s = this.DARK_MODE ? "light" : "dark",
						n = window[a].SKIN,
						o = {
							btn_update: function() {
							...
							hlp_update_images: function() {
								e.GLOSSY || (e.$gHelpLayer.find("span[name=home_link]").css({
									background: "url(" + n.HOME_LINK[s].toDataURL() + ") no-repeat center"
								}), e.$gHelpLayer.find("span[name=help_arrow]").css({
									background: "url(" + n.HELP_ARROW[s].toDataURL() + ") no-repeat center"
								}))
							}
						};
					if(this.GLOSSY) {
					...
					if(this.DARK_MODE) {
						...
						this.$gBookBackground.addClass("light"), i.addClass("light"), this.$gIconsPanel.addClass("light"), this.GLOSSY ? this.toggleSkinHlpLayerGLTo("dark") : (this.$gHelpLayer.addClass("light"), this.$gHelpLayer.find(".hlpLeftSide").css({
							background: "url(" + h.toDataURL() + ")"
						}), this.$gHelpLayer.find(".hlpRightSide").css({
						...
						this.DARK_MODE = !0;
						var h = this.getHlpLayerBgImage();
						this.$gBookBackground.removeClass("light"), i.removeClass("light"), this.$gIconsPanel.removeClass("light"), this.GLOSSY ? this.toggleSkinHlpLayerGLTo("light") : (this.$gHelpLayer.removeClass("light"), this.$gHelpLayer.find(".hlpLeftSide").css({
							background: "url(" + h.toDataURL() + ")"
						}), this.$gHelpLayer.find(".hlpRightSide").css({
							background: "url(" + h.toDataURL() + ")"
						})), o.hlp_update_images(), o.btn_update()
				...
				pageNumbersCreate: function(e) {
					var i = this.divNames.book_page_numbers,
						s = this.CFG.ICONS_PANEL[e],
						a = s.pageNumbers.width,
						n = s.pageNumbers.fontSize,
				...
				pageNumbersChange: function() {
					var e = this.divNames.book_page_numbers,
						i = this.CURRENT.spread,
						s = this.getTotalSheets(),
						a = i + "&nbsp;/&nbsp;" + s;
				...
				buttonCreate: function(e, i, s) {
					...
					var n = this,
						e = e.toLowerCase(),
						o = s ? s.mode : this.DARK_MODE ? "dark" : "light";
					if(this.LARGE_ICONS) var h = window[a].SKIN.ICONS,
						r = this.BTNS_OFFSET,
						d = this.CFG.ICONS_SIZE.big,
						l = ['<table width="100%" cellpadding="0" cellspacing="0">', '<tr><td height="' + d + 'px" align="center">', '<div class="btn_image" style="width:' + d + "px;height:" + d + 'px;" ><img src="' + this.GIF + '"></div>', "</td></tr>", '<tr><td align="center" class="btn_title">-</td></tr></table>'].join("");
					else var h = window[a].SKIN.ICONS_MINI,
						r = this.BTNS_MINI_OFFSET,
						d = this.CFG.ICONS_SIZE.small,
					...
					return E.hover(function() {
						...
						var t = g(),
							e = n.DARK_MODE ? "dark" : "light",
							i = "url(" + h[e].toDataURL() + ") no-repeat " + t.offset + " 0px",
							s = this.find(".btn_image").css({
								background: i,
								opacity: A("end")
							}).attr({
								title: t.lngTitle
				...
				zoomIn: function(e) {
					...
					var s = this.divNames.zoom_layer,
						a = t("#" + s);
				...
				showZoomContent: function(t) {
					var e = this,
						...
						g = {
							...
							width: this.CFG.BOOK_UNSCALED.width * l
						},
						A = g.width < d.width && g.height < d.height,
						p = {
							vertical: (g.height + 2 * h - d.height) / 100,
							horizontal: (g.width + 2 * h - d.width) / 100
						},
						c = "",
						f = this.divNames.zoom_content,
						E = this.divNames.zoom_behavior_section,
						S = this.divNames.zoom_title_spread,
					...
					var v = this.$gZoomLayer.find("." + f),
						_ = this.$gZoomLayer.find("." + E).css({
							height: d.height
						}),
						T = (this.$gZoomLayer.find("." + S).css({
							height: r
						}), this.$gZoomLayer.find("." + u)),
						O = {
							...
							calculateCoord: function(t) {
								var e = {
									...
									i = {
										vertical: 100 / (d.height / e.y),
										horizontal: 100 / (d.width / e.x)
									},
									s = p.vertical * i.vertical * -1 + h,
									a = p.horizontal * i.horizontal * -1 + h;
								return {
									top: s,
									left: a
								}
							...
							contentToCenter: function() {
								var e = 20,
									i = parseFloat(c.css("marginLeft")) || 0,
									s = parseFloat(c.css("marginTop")) || 0;
								if(t && !A) var a = this.calculateCoord(t),
									n = a.left,
									o = a.top;
								else var n = (g.width - d.width) / 2 * -1,
							...
							behaviors: function() {
								...
								_.mousemove(function(t) {
									A || O.moveContent(i.calculateCoord({
										x: t.pageX,
										y: t.pageY
									}))
								}).hover(function() {
							...
							recurseAnimation: function(t, i, s, a) {
								e.TMR_ZOOMPAGES_ANI = setTimeout(function() {
									i++;
									var e = i / t,
										n = 1 - (1 - Math.sin(Math.acos(1 - e))),
										o = 100 * n,
										h = s[1] - (s[1] - a[1]) / 100 * o,
										r = s[0] - (s[0] - a[0]) / 100 * o;
									c.css({
										margin: h + "px 0px 0px " + r + "px"
									}), t > i && O.recurseAnimation(t, i, s, a)
								}, 5)
							},
							...
							insertImageToZoomPage: function(t) {
								var i = "left" == t.side ? ".page_back" : ".page_front",
									a = "left" == t.side ? s - 1 : s,
								...
								if(e.CFG.DIVIDE_IMAGES) {
									...
									c.find(i).css({
										background: 'url("' + n.toDataURL() + '") ' + o + "px 0px no-repeat"
									})
								} else c.find(i).css({
									background: 'url("' + n.toDataURL() + '") no-repeat'
								})
							},
				...
				slopeAnimate: function() {
					...
					var e = this.CFG.ARR_SLOPE_ANGLES.length,
						i = t.CURRENT.slope_mode,
						s = this.CFG.ARR_SLOPE_ANGLES[i];
					this.CURRENT.slope_mode = e > i + 1 ? i + 1 : 0, this.CFG.START_SLOPE_MODE = this.CURRENT.slope_mode;
					var a = this.CFG.ARR_SLOPE_ANGLES[this.CURRENT.slope_mode],
						n = a > s ? 1 : -1,
						o = function() {
				...
				slopeAnimateTo: function(t) {
					...
					if(this.ANIMATION_SLOPE) return !1;
					this.ANIMATION_SLOPE = !0;
					var i = this.CFG.ARR_SLOPE_ANGLES.length,
						s = e.CURRENT.slope_mode,
						a = this.CFG.ARR_SLOPE_ANGLES[s];
					if("up" === t) {
					...
					var n = this.CFG.ARR_SLOPE_ANGLES[this.CURRENT.slope_mode],
						o = n > a ? 1 : -1,
						h = function() {
				...
				fit3dDataToBookSize: function() {
					...
					var a = this.getIconsPanelHeight(),
						n = {
					...
					if(o.w > n.w || o.h > n.h) var h = n.w / o.w,
						r = n.h / o.h,
						d = Math.min(h, r);
					else var d = 1;
					...
					var l = {
							width: 800,
							height: Math.round(800 * i.height / i.width)
						},
						g = this.FRAMES[0][0].bezie.length,
						A = Math.abs(this.FRAMES[0][0].bezie[g - 1].x),
						p = i.width / A,
						c = l.width / A,
						f = function(t, e) {
							for(var i = 0; i < t.length; i++)
								for(var s = t[i].edges3d, a = 0; a < s.length; a++) s[a].width *= e, s[a].x *= e, s[a].z *= e
						};
				...
				createGlHlpLayer: function() {
					var t = this,
						e = this.CFG.BOOK_SIZE,
						i = this.CFG.BOOK_GL_SIZE,
						s = {
						...
						r = {
							...
							hlpLeftImage: function(e) {
								var i = t.getLNG("hlpClickToOpen"),
									s = t.getLNG("hlpUseMousewheelGL"),
						...
						g = r.PlainTextured({
							src: r.backgroundImage("left", "dark"),
							side: "left",
							skin: "dark",
							onReady: function(e) {
								e.position.y = i.height * t.CFG.ROTATE_CENTER_OFFSET_GL, e.position.z = -5, e.position.x = .5 * -i.width, t.gGL.bookBase.add(e)
				...
				calculateSheets: function() {
					if(!this.ALLSHEETS || !this.ALLSHEETS.length) {
						...
						if(this.CFG.DIVIDE_IMAGES) {
							...
							var e = 2 * this.ARR_PAGES_SRC.length,
								i = [];
				...
				createSelfShadow: function(t, e) {
					...
					var s = i.getContext("2d"),
						a = s.createLinearGradient(0, 0, i.width, 0);
				...
				behavior: function() {
					...
					var s = {
							...
							show: function() {
								var t = e.STAGE_BOUNDING_BOX,
									i = e.MOUSE.startDragCoord;
								e.$gZoomWaiter.css({
									top: i.y - 30 - t.top,
									left: i.x - 30 - t.left
								}).show()
					...
					o.unbind(), o.swipeMePls({
						...
						if(e.startDrag) {
							...
							var s = Math.abs(e.MOUSE.startDragCoord.x - e.MOUSE.lastCoord.x),
								a = Math.round(s / e.CFG.DRAG_PARAM.px_per_persent),
								o = a + e.START_DRAG_FRAME || 0;
				...
				calculateBounds: function() {
					var e = this,
						...
						h = e.CFG.PASPARTU_BEHAVIORS_PARAM.height * e.GSCALE,
						r = h * i.overlay_pr,
				...
				stageToCenter: function() {
					...
					this.STAGE && (this.tmrToCenter && clearTimeout(this.tmrToCenter), this.$gStage.hide(), this.tmrToCenter = setTimeout(function() {
						...
						var e = t.ICONS_PANEL_BOUNDS,
							i = {
								width: t.$gBookLoader.width(),
								height: t.$gBookLoader.height()
							},
							s = {
								width: t.$gSaveMenu.width(),
								height: t.$gSaveMenu.height()
							},
							a = t.STAGE;
						t.$gIconsPanel.css({
							top: e.top,
				...
				correctTargetHeight: function() {
					...
					}), this.$TARGET.attr("onebook3d", this.PRE_NAME)), window[a].buildNextBook()
				},
				drag_sheet: function(t) {
					var t = 100 > t ? t : 100,
						e = this.CURRENT.dragged_sheet;
					if(this.GLOSSY) var i = this.getSheetByIndex(e),
						s = this.getEdgesFormSheet(e),
						a = "front" == this.startDrag ? 1 : 0,
						n = {
							gl_sheet: i,
							gl_edges: s,
							forward: a
						...
						var o = this.getSheetByIndex(e),
							h = this.getEdgesFormSheet(e),
							a = "front" == this.startDrag ? 1 : 0,
							n = {
								sheet: e,
								$edges: h,
				...
				moveSheetTo: function(t, e) {
					...
					if(this.GLOSSY) {
						...
						var d = this.$gHelpLayer.find(".hlpLeftSide"),
							l = this.$gHelpLayer.find(".hlpRightSide"),
							r = this.getTotalSheets();
						i.startDrag && !s && a ? d.css({
							opacity: (100 - t) / 100
						}) : i.startDrag && s === r - 1 && !a && l.css({
							opacity: (100 - t) / 100
						})
					}
					if(this.GLOSSY) {
				...
				calculateSideAndFrame: function(t) {
					var e = this;
					try {
						var i = t.y - this.STAGE_CENTER.top,
							s = Math.abs(i),
							a = t.x - this.STAGE_CENTER.left,
							n = this.STAGE_HALF.width - this.CFG.BOOK_SIZE.width,
							o = this.CFG.BOOK_SIZE.width + Math.floor(.8 * n),
							h = this.STAGE_HALF.width - o,
							r = h / 100,
							d = Math.abs(a),
							l = {
				...
				startOffsetDepth: function(e, i) {
					...
					if(this.GLOSSY) {
						...
						if(e) a.sidesRight[0].mesh.position.z = 1 * s.CFG.GL_DEPTH_STEP, t.each(a.sidesRight, function(t) {
							...
							var n = a.sidesLeft.length,
								o = 1;
							if(n)
								for(var h = n; h > 0; h--) a.sidesLeft[h - 1].mesh.position.z = o * s.CFG.GL_DEPTH_STEP, o -= 1
						}
					} else {
						...
						if(e) d.eq(0).css({
							...
							var n = r.size(),
								o = 1;
							if(n)
								for(var h = n; h > 0; h--) r.eq(h - 1).css({
					...
					if(this.GLOSSY) {
						...
						if(this.GL_ARR_SHEETS.length)
							if(t) {
								...
								if(a)
									for(var n = a; n > 0; n--) s.sidesLeft[a - n].mesh.position.z = -1 * n * i.CFG.GL_DEPTH_STEP;
								s.sidesRight[0].mesh.position.z = 0
							} else {
								var a = s.sidesRight.length,
									o = -1 * a;
								if(a)
									for(var n = a; n > 0; n--) s.sidesRight[n - 1].mesh.position.z = o * i.CFG.GL_DEPTH_STEP, o += 1;
						...
						if(t) {
							var a = h.size();
							if(a)
								for(var n = a; n > 0; n--) h.eq(a - n).css({
							...
							var a = r.size(),
								o = -1 * a;
							if(a)
								for(var n = a; n > 0; n--) r.eq(n - 1).css({
									"-webkit-transform": "translate3d(0px,0px," + o + "px)",
									"-moz-transform": "translate3d(0px,0px," + o + "px)",
									"-ms-transform": "translate3d(0px,0px," + o + "px)"
								}), o += 1;
				...
				animateSheet: function(t) {
					if(this.isDirectionCorrect(t)) {
						var e = this,
							i = "next" === t ? this.CURRENT.spread : this.CURRENT.spread - 1,
							s = "next" === t ? 1 : 0,
							a = function() {
								var t = h + 1,
									e = 100 - t,
						...
						var o = 4,
							h = this.CURRENT.frame,
							r = s ? this.CURRENT.spread : this.CURRENT.spread - 1,
							d = this.ANIMATION.SPEED,
						...
						else var c = e.getEdgesFormSheet(r),
							p = {
								sheet: r,
								$edges: c,
								forward: s
							};
						var f = 0 === this.CURRENT.spread & s,
							E = 1 === this.CURRENT.spread & !s,
							S = this.CURRENT.spread === l & !s,
							u = this.CURRENT.spread === l - 1 & s;
						if(e.GLOSSY) var R = this.getHlpLayerGL();
						else var m = this.$gHelpLayer.find(".hlpLeftSide"),
							v = this.$gHelpLayer.find(".hlpRightSide");
						var _ = function() {
							101 > g ? (e.moveSheetTo(g, p), e.GLOSSY ? R.left && R.right && (f && (R.left.material.opacity = (100 - g) / 100), E && (R.left.material.opacity = g / 100), S && (R.right.material.opacity = (100 - g) / 100), u && (R.right.material.opacity = g / 100)) : (f && m.css({
								opacity: (100 - g) / 100
							}), E && m.css({
								opacity: g / 100
							}), S && v.css({
				...
				switchCurrents: function(t) {
					{
						var e = this,
							i = t.sheet,
							s = t.forward,
							a = s ? 10 * (i + 1) : 10 * (this.ALLSHEETS.length - i);
						this.CURRENT.spread
					}
					if(this.LAST.forward = s, this.LAST.moved.forward = s ? 0 : 1, e.GLOSSY) {
						var n = s ? 180 : 0,
							o = this.getSheetByIndex(i);
						o.sideClass = "sideLeft" === o.sideClass ? "sideRight" : "sideLeft"
					} else {
						var n = s ? 180 : 0,
							h = this.getSheetByIndex(i),
							r = h.find(".wholeimage"),
							d = h.find(".all_edges");
						h.css({
							zIndex: a
						}).toggleClass("side-right").toggleClass("side-left"), r.css({
					...
					var l = this.CURRENT.spread,
						g = this.ARR_PAGES_SRC.length;
					if(0 === l);
					else if(2 * l === g);
					...
					if(A && this.isDirectionCorrect(A)) e.GLOSSY ? (e.ORDER_TO_ANIMATE = !1, e.animateSheet(A)) : r.fadeIn("fast", function() {
						...
						var p = e.CURRENT.spread === e.getTotalSheets(),
							c = e.$G_ARR_BUTTONS.togglebook;
						...
						var t = e.CURRENT.spread === e.getTotalSheets(),
							i = e.$G_ARR_BUTTONS.togglebook;
				...
				build_background: function() {
					var e = this.divNames.background,
						i = t("#" + e);
					i.size() && i.remove();
				...
				buildSaveMenuLayer: function() {
					...
					var e = this,
						i = this.divNames.save_menu,
						s = 230,
						a = .5 * this.STAGE.width,
						a = s > a ? s : a,
						n = this.CFG.ZINDEX.bookSaveMenu,
						o = function() {
							var t = document.createElement("canvas");
				...
				buildHelpLayer: function() {
					var e = this,
						...
						s = this.getLNG("hlpGoToHomelink"),
						n = this.getLNG("hlpClickToOpen"),
						o = this.getLNG("hlpUseMousewheel"),
						h = this.getLNG("toStart"),
						r = i.width < 150 ? 'class="middleSize"' : "",
						d = i.height < 130,
					...
					var c = this.START_PAGE < 2 ? 1 : 0,
						f = this.START_FROM_END ? 1 : 0,
					...
					var e = this,
						...
						s = {
							...
							buildColorSheet: function(t, a) {
								var n = 0,
									o = t.num === l || t.num > l ? 1 : 0,
									h = e.FRAMES_GL_SCALED[o][n].edges3d,
									r = e.CFG.BOOK_GL_SIZE.height,
									d = t.zindex3d,
									g = 2 * h.length;
						...
						n = function(s) {
							if(!e.SHEETS_WAS_BUILT[s]) {
								...
								var a = s === l || s > l,
									n = a ? 10 * (e.ALLSHEETS.length - s) : 10 * (s + 1),
									o = a ? "side-right" : "side-left",
									h = a ? "right" : "left",
									r = -1 * f[h],
						...
						o = 1.2,
						h = this.LAST.forward,
						r = (this.GIF, this.CFG.NUMBEROF_PRELOAD_SHEETS),
						d = this.ALLSHEETS.length,
						l = this.CURRENT.spread,
						g = l == d,
						A = l - r,
						p = l + r;
				...
				getColorsBySheet: function(t) {
					var e = this.CFG.PAGE_DEFAULT_COLOR,
						i = this.ALLSHEETS[t].front;
					i = i.indexOf(".") > -1 ? e : i;
				...
				createImageFromColor: function(t, e, i) {
					var s = this,
						a = function() {
							var t = document.createElement("canvas"),
								e = s.CFG.BOOK_SIZE.width,
								i = s.CFG.BOOK_SIZE.height;
				...
				loadImageForPage: function(t, e, i) {
					...
					this.NOW_LOADING_COUNTER++, this.IMG_NOW_LOADING[a] = new Image, this.IMG_NOW_LOADING[a].onload = function() {
						s.NOW_LOADING_COUNTER--, s.update_queue_loaded_images({
							img: this,
							src: t,
							sheet: e,
							page: i
				...
				buildResizedImage: function(t, e, i, s) {
					var a = this.getString([76, 73, 77, 73, 84, 69, 68, 0, 76, 73, 71, 72, 84, 0, 86, 69, 82, 83, 73, 79, 78]),
						n = i.img,
						o = i.page > 0 ? "back" : "front",
						h = this.getColorsBySheet(i.sheet),
						r = i.sheet,
					...
					var p = this.CFG.BORDER * e,
						c = d.width - 2 * p,
						f = d.height - 2 * p;
					if(n.width > c || n.height > f) var E = c / n.width,
						S = f / n.height,
						u = Math.min(E, S),
						R = E == u,
						m = R ? c : Math.floor(n.width * u),
						v = R ? Math.floor(n.height * u) : f;
					else var m = n.width,
						v = n.height;
					A.drawImage(n, p, p, m, v);
					var _ = this.getTotalSheets(),
						T = 0 === i.sheet && 0 === i.page,
						O = i.sheet === _ - 1 && 1 === i.page;
					if(s) {
					...
					var s = {
							...
							cutImagesAndInsertIntoEdges: function(e) {
								...
								if(i.GLOSSY) {
									...
									var m = l.find(".wholeimage ." + r + "_image"),
										v = m.find("div"),
										_ = l.find("." + r),
										T = "back" === r ? -1 : 1;
									v.css({
										background: "url(" + e.toDataURL() + ") " + T + "px 0px no-repeat"
									}), m.css({
										background: "url(" + e.toDataURL() + ") 0px 0px no-repeat"
									}), _.each(function(e) {
										t(this).css({
						...
						a = i.FRAMES[0][this.FIRST_FRAME].edges3d,
						n = [],
						o = (e.img, e.page),
						h = e.sheet,
						r = o > 0 ? "back" : "front";
					if(this.GLOSSY) var d = i.GL_ARR_SHEETS[h];
					else var l = this.$ARR_SHEETS[h];
				...
				saveImagesAs: function() {
					var e = this,
						...
						a = 0 == this.CURRENT.spread,
						n = this.CURRENT.spread == this.getTotalSheets(),
			...
			o = {
				...
				buildAllFramesFrom: function(e) {
					...
					for(var s = 1; s < e.length; s++)
						for(var a = e[s - 1], n = e[s], o = n.frame - a.frame, h = 1; o > h; h++) {
				...
				calculateCenters: function(e) {
					...
					return t.each(e, function(t) {
						if(!(t < i.FIRST_KEYFRAME))
							for(var e = this.line, t = 0; t < e.length; t++) {
								var s = e[t].ang,
									a = s > 0 ? 1 : -1,
									n = s * a,
									o = e[t].center,
				...
				calculateBezie: function() {
					var e = this.ALL_FRAMES,
						i = this.CFG.BEZIE_ROUGH;
					t.each(e, function() {
						var t = this.line;
						if(!(t.length < 3)) {
							for(var e = [{
				...
				calculateEdgeSkewY: function() {
					var e = this.CFG.BOOK_HEIGHT,
						i = this.CFG.YSCALE,
						s = this.CFG.YSCALETOP,
						a = this.ALL_FRAMES;
					t.each(a, function() {
